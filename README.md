...
> **This is the `verify-sentry-2.61.0-fix` branch**, which pins
> `sentry-sdk==2.61.0` to confirm the deadlock is resolved without any
> workarounds. See the [`main`](../../tree/main) branch for the deadlock
> reproduction with latest packages, or the
> [`production-versions`](../../tree/production-versions) branch for the
> reproduction pinned to our exact production versions.
...
## Resolution: `sentry-sdk` 2.61.0

This deadlock was fixed in [`sentry-sdk` 2.61.0](https://github.com/getsentry/sentry-python/issues/5586).
The serializer no longer calls `__iter__` on non-builtin `Sequence` subclasses
(like OTel's `BoundedList`). Instead, it uses `__repr__`, which does not acquire
any locks — breaking the deadlock chain at step 5.

The [`verify-sentry-2.61.0-fix`](../../tree/verify-sentry-2.61.0-fix)
branch pins `sentry-sdk==2.61.0` and confirms the deadlock no longer occurs
without any workarounds (no `--with-fix`, no `--ignore-asyncio-logger`).

With this fix, the workarounds above are no longer necessary.

## Versions tested

This branch pins `sentry-sdk==2.61.0` alongside the production OTel versions
to verify the fix. The OTel code path is identical to `production-versions`
— only the Sentry serializer behavior differs:
...
| **`verify-sentry-2.61.0-fix`** (this branch) | 2.61.0 | 1.20.0 | Same as `production-versions` — **no deadlock** | `BatchSpanProcessor` |
